	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"vlan_id": {
				Type:         schema.TypeInt,
				Required:     true,
				ValidateFunc: validateVirtualBorderRouterVlanId,
			},
			"local_gateway_ip": {
				Type:     schema.TypeString,
				Required: true,
			...
			"status": {
				Type:         schema.TypeString,
				Optional:     true,
				ValidateFunc: validateVBRStatus,
				Default:      "active",
			},
		},
	}
...
func resourceAliyunVirtualBorderRouterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if !d.IsNewResource() && d.HasChange("status") {
		d.SetPartial("status")
		if d.Get("status").(string) == string("active") {
			terminateRequest := vpc.CreateTerminateVirtualBorderRouterRequest()
			terminateRequest.RegionId = client.RegionId
			terminateRequest.VbrId = d.Id()
			request.ClientToken = buildClientToken(request.GetActionName())
			raw, err := client.WithVpcClient(func(vpcClient *vpc.Client) (interface{}, error) {
				return vpcClient.TerminateVirtualBorderRouter(terminateRequest)
			})
			if err != nil {
				return WrapErrorf(err, DefaultErrorMsg, d.Id(), terminateRequest.GetActionName(), AlibabaCloudSdkGoERROR)
			}
			addDebug(terminateRequest.GetActionName(), raw, terminateRequest.RpcRequest, terminateRequest)
		} else if d.Get("status").(string) == string("terminated") {
			recoverRequest := vpc.CreateRecoverVirtualBorderRouterRequest()
			recoverRequest.RegionId = client.RegionId
			recoverRequest.VbrId = d.Id()
			request.ClientToken = buildClientToken(request.GetActionName())
			raw, err := client.WithVpcClient(func(vpcClient *vpc.Client) (interface{}, error) {
				return vpcClient.RecoverVirtualBorderRouter(recoverRequest)
			})
			if err != nil {
				return WrapErrorf(err, DefaultErrorMsg, d.Id(), recoverRequest.GetActionName(), AlibabaCloudSdkGoERROR)
			}
			addDebug(recoverRequest.GetActionName(), raw, recoverRequest.RpcRequest, recoverRequest)
		}
	}
...
func resourceAliyunVirtualBorderRouterDelete(d *schema.ResourceData, meta interface{}) error {
	...
	err := resource.Retry(5*time.Minute, func() *resource.RetryError {
		...
		if err != nil {
			if IsExceptedError(err, InvalidVbrIdNotFound) {
				return nil
			}
			return resource.RetryableError(err)
		}
